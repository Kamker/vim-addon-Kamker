...
fu! GraphvizCompile(tool, output)
	...
	if has('win32')
		let cmd = 'set makeprg='.a:tool.'\ -T'.a:output.'\ '
		let cmd .= substitute(g:WMGraphviz_shelloptions, ' ', '\\ ', 'g').'\ \"'
		let cmd .= escape(expand("%:p"), '\ ').'\"\ -o\ \"'
		let cmd .= escape(expand("%:p:r"), '\ ').'.'.a:output.'\"'
		exec cmd
		silent make
	else
...
fu! GraphvizShow()
	...
	if !has('win32')
		...
		silent!!cd /d %:p:h & cmd /cstart chrome "%:p:r.png"
	endif
endfu
